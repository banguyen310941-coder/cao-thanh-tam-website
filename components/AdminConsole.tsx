...
import { FileText, Home, Images, LayoutDashboard, Plus, Save, Trash2, Upload, Users, X, ChevronLeft, ChevronRight, Sparkles } from "lucide-react";
...
async function prepareImage(input:File):Promise<File>{
 ...
 for(let attempt=0;attempt<8;attempt++){
  ...
  const ctx=canvas.getContext("2d");if(!ctx){bitmap.close();throw new Error("Không thể xử lý ảnh")}
  ctx.drawImage(bitmap,0,0,width,height);
  blob=await new Promise<Blob|null>(resolve=>canvas.toBlob(resolve,"image/jpeg",quality));
  if(blob&&blob.size<=TARGET_BYTES)break;
  quality=Math.max(.5,quality-.08);maxSide=Math.max(1100,maxSide-150);
 }
 bitmap.close();
...
export default function AdminConsole(){
 ...
 async function load(){const r=await fetch("/api/cms",{cache:"no-store"});const j=await r.json();if(j.data){setData(j.data);setStorage(!!j.storage);setSelectedAlbum(v=>j.data.albums.some((a:Album)=>a.id===v)?v:j.data.albums?.[0]?.id||"")}setReady(true)}
 useEffect(()=>{load()},[]);
 ...
 async function uploadFiles(files:FileList|null){
  ...
  for(let i=0;i<chosen.length;i++){
   ...
   try{
    ...
    if(file.size>1024*1024){errors.push(`${original.name}: sau nén vẫn trên 1MB`);continue}
  ...
  flash(`Đã tải ${ok}/${chosen.length} ảnh${errors.length?`. ${errors.length} ảnh chưa tải được`:""}`);
 }
 async function cleanup(){if(!confirm("Xóa ảnh trên 1MB và xóa các ảnh trùng hoàn toàn trong toàn bộ album?"))return;setCleaning(true);const r=await fetch("/api/cms/cleanup",{method:"POST"});const j=await r.json().catch(()=>({}));setCleaning(false);if(!r.ok){flash(`Không thể dọn ảnh: ${j.error||r.status}`);return}await load();flash(`Đã dọn kho ảnh: xóa ${j.deletedOversized} ảnh >1MB và ${j.deletedDuplicates} ảnh trùng. Còn ${j.remaining} ảnh.`)}
 async function deletePhoto(p:Photo){if(!confirm(`Xóa ảnh ${p.name}?`))return;const r=await fetch("/api/cms/photo",{method:"DELETE",headers:{"content-type":"application/json"},body:JSON.stringify({url:p.url})});if(r.ok){await load();flash("Đã xóa ảnh") }else flash("Không thể xóa ảnh")}
 function renameAlbum(a:Album){const title=prompt("Tên album mới",a.title);if(!title||title===a.title)return;save({...data,albums:data.albums.map(x=>x.id===a.id?{...x,title}:x)},"Đã đổi tên album")}
 ...
 return <main className="admin-shell">
  ...
  {tab==="albums"&&<div className="admin-card"><div className="admin-card-head"><div><h2>Album hình ảnh</h2><p>Ảnh mới sẽ tự nén xuống dưới 1MB. Có thể click ảnh để xem lớn.</p></div><div style={{display:"flex",gap:8}}><button className="admin-primary" disabled={cleaning} onClick={cleanup}><Sparkles size={16}/>{cleaning?"Đang dọn...":"Dọn ảnh trùng & >1MB"}</button><button className="admin-primary" onClick={async()=>{const title=prompt("Tên album mới");if(!title)return;const next={...data,albums:[...data.albums,{id:`album-${Date.now()}`,title,createdAt:new Date().toISOString()}]};if(await save(next,"Đã tạo album"))setSelectedAlbum(next.albums.at(-1)?.id||"")}}><Plus size={16}/>Thêm album</button></div></div>
   <div className="admin-upload-box"><div><b>Tải ảnh hàng loạt</b><p>JPG, PNG, WEBP, HEIC/HEIF. Mỗi ảnh sau xử lý sẽ dưới 1MB.</p>{uploadProgress&&<p><b>{uploadProgress}</b></p>}</div><select value={selectedAlbum} onChange={e=>setSelectedAlbum(e.target.value)}>{data.albums.map(a=><option key={a.id} value={a.id}>{a.title}</option>)}</select><input ref={fileRef} type="file" accept="image/*,.heic,.heif" multiple onChange={e=>uploadFiles(e.target.files)} hidden/><button className="admin-primary" disabled={uploading||!storage||!selectedAlbum} onClick={()=>fileRef.current?.click()}><Upload size={16}/>{uploading?"Đang tải...":"Chọn nhiều ảnh"}</button></div>
   ...
   <div className="admin-album-grid">{data.albums.map(a=>{const photos=data.photos.filter(p=>p.albumId===a.id);return <article key={a.id} className={selectedAlbum===a.id?"selected":""} onClick={()=>setSelectedAlbum(a.id)} onDoubleClick={()=>renameAlbum(a)} title="Nhấp đúp để đổi tên album"><div className="admin-album-cover">{photos[0]?<img src={photos[0].url} alt={a.title}/>:<Images size={28}/>}</div><div><b>{a.title}</b><span>{photos.length} hình ảnh</span></div><button className="icon-btn" onClick={async e=>{e.stopPropagation();if(!confirm(`Xóa album ${a.title}?`))return;await save({...data,albums:data.albums.filter(x=>x.id!==a.id)},"Đã xóa album")}}><Trash2 size={16}/></button></article>})}</div>
   {selectedAlbum&&<><h3 className="admin-gallery-title">Ảnh trong album</h3><div className="admin-photo-grid">{albumPhotos.map((p,i)=><figure key={p.id} onClick={()=>setLightbox(i)} style={{cursor:"zoom-in"}}><img src={p.url} alt={p.name}/><figcaption>{p.name}</figcaption><button onClick={e=>{e.stopPropagation();deletePhoto(p)}}><Trash2 size={14}/></button></figure>)}</div></>}
  </div>}
  ...
  {lightbox!==null&&albumPhotos[lightbox]&&<div className="admin-lightbox" onClick={()=>setLightbox(null)}><button className="admin-lightbox-close" onClick={()=>setLightbox(null)}><X/></button><button className="admin-lightbox-prev" onClick={e=>{e.stopPropagation();setLightbox((lightbox-1+albumPhotos.length)%albumPhotos.length)}}><ChevronLeft/></button><img src={albumPhotos[lightbox].url} alt={albumPhotos[lightbox].name} onClick={e=>e.stopPropagation()}/><div className="admin-lightbox-caption">{albumPhotos[lightbox].name} · {lightbox+1}/{albumPhotos.length}</div><button className="admin-lightbox-next" onClick={e=>{e.stopPropagation();setLightbox((lightbox+1)%albumPhotos.length)}}><ChevronRight/></button></div>}
 </main>
}
