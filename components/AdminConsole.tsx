...
export default function AdminConsole(){
 ...
 async function load(){const r=await fetch("/api/cms",{cache:"no-store"});const j=await r.json();if(j.data){setData(j.data);setStorage(!!j.storage);setSelectedAlbum(j.data.albums?.[0]?.id||"")}setReady(true)}
 useEffect(()=>{load()},[]);
 ...
 async function save(next=data,msg="Đã lưu thay đổi"){setSaving(true);const r=await fetch("/api/cms",{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify(next)});const j=await r.json();setSaving(false);if(!r.ok){flash(j.error==="BLOB_NOT_CONFIGURED"?"Chưa kết nối Vercel Blob":"Không thể lưu dữ liệu");return false}setData(next);flash(msg);return true}
 async function uploadFiles(files:FileList|null){if(!files?.length||!selectedAlbum)return;setUploading(true);let ok=0,fail=0;for(const file of Array.from(files)){if(file.size>4*1024*1024){fail++;continue}const form=new FormData();form.append("albumId",selectedAlbum);form.append("file",file);const r=await fetch("/api/cms/upload",{method:"POST",body:form});if(r.ok)ok++;else fail++}await load();setUploading(false);if(fileRef.current)fileRef.current.value="";flash(`Đã tải ${ok} ảnh${fail?`, ${fail} ảnh lỗi/quá 4MB`:""}`)}
 ...
 return <main className="admin-shell">
  ...
  {tab==="albums"&&<div className="admin-card"><div className="admin-card-head"><div><h2>Album hình ảnh</h2><p>Chọn nhiều ảnh cùng lúc. Ảnh tải lên sẽ hiển thị tại thư viện trên trang chủ.</p></div><button className="admin-primary" onClick={async()=>{const title=prompt("Tên album mới");if(!title)return;const next={...data,albums:[...data.albums,{id:`album-${Date.now()}`,title,createdAt:new Date().toISOString()}]};if(await save(next,"Đã tạo album"))setSelectedAlbum(next.albums.at(-1)?.id||"")}}><Plus size={16}/>Thêm album</button></div>
   <div className="admin-upload-box"><div><b>Tải ảnh hàng loạt</b><p>Có thể chọn nhiều file JPG, PNG, WEBP. Mỗi ảnh tối đa 4MB.</p></div><select value={selectedAlbum} onChange={e=>setSelectedAlbum(e.target.value)}>{data.albums.map(a=><option key={a.id} value={a.id}>{a.title}</option>)}</select><input ref={fileRef} type="file" accept="image/*" multiple onChange={e=>uploadFiles(e.target.files)} hidden/><button className="admin-primary" disabled={uploading||!storage||!selectedAlbum} onClick={()=>fileRef.current?.click()}><Upload size={16}/>{uploading?"Đang tải...":"Chọn nhiều ảnh"}</button></div>
